...
describe('Register', function () {
  ...
  it('create a user', function () {
    return this.session.post('/register').send(this.newUserData).expect(302).expect('Location', '/');
  });

  it('username exists raises failure', async function () {
    await this.session.post('/register').send(this.newUserData).expect(302).expect('Location', '/');
  ...
  it('create user, logout and log back in', async function () {
    ...
    await this.session.post('/login').send(logInData).expect(302).expect('Location', '/');
  });
});
